...
var getRectFromCoords = function getRectFromCoords(coords) {
  return {
    top: coords.y - 1,
    right: coords.x + 1,
    bottom: coords.y + 1,
...
var getActualBoundingClientRect = function getActualBoundingClientRect(node) {
...
var getScrollParents = function getScrollParents(element) {
  ...
  parents.push(element.ownerDocument.body); // If the node is within a frame, account for the parent window scroll
...
var getBounds = function getBounds(element) {
  var doc = document;
  ...
  var box = getActualBoundingClientRect(currentElement);
  ...
  box.bottom = doc.body.clientHeight - box.height - box.top; // round origin
...
var getBoundingRect = function getBoundingRect(boundingElement) {
  ...
  var bounds = getBounds(boundingElement);
  var style = getComputedStyle(boundingElement);
  var rect = {
    top: bounds.top,
  ...
  if (boundingElement.ownerDocument !== document) {
    var parentView = boundingElement.ownerDocument.defaultView;
  ...
  BOUNDS_FORMAT.forEach(function (side) {
    var subSide = side[0].toUpperCase() + side.substr(1);

    if (subSide === 'Top' || subSide === 'Left') {
      rect[side] += parseFloat(style["border".concat(subSide, "Width")]);
    } else {
      rect[side] -= parseFloat(style["border".concat(subSide, "Width")]);
    }
  }); // round origin
...
var getDirectionalAttachment = function getDirectionalAttachment(attachment, isRTL) {
...
var getDirectionalOffset = function getDirectionalOffset(offset, isRTL) {
  if (isRTL) {
    return {
...
var mirrorOffset = function mirrorOffset(offset, attachment) {
  if (attachment.vertical !== 'middle') {
    return {
      vertical: -offset.vertical,
      horizontal: offset.horizontal
...
var rotateContentAttachment = function rotateContentAttachment(attachment, angle) {
  var rAttachment = {};
...
var isValidPositions = function isValidPositions(positions, boundingRect) {
  var content = positions.content;
...
var getTargetPosition = function getTargetPosition(target) {
  var targetCoords = {};
  var targetTop = target.rect.top;
  var targetHeight = target.rect.height;
  var targetLeft = target.rect.left;
...
var mirrorTargetPosition = function mirrorTargetPosition(target, contentAttachment) {
  var mirroredTarget = {
    rect: target.rect,
    attachment: target.attachment,
...
var getRelativePositions = function getRelativePositions(content, target, margin) {
  var contentCoords = {};
...
var getMirroredPositions = function getMirroredPositions(positions, margin) {
  var newContent = {
...
var getRotatedPositions = function getRotatedPositions(positions, margin, rotation) {
  var newContent = {
    offset: {
      vertical: 0,
      horizontal: 0
...
var getAlternatePositions = function getAlternatePositions(positions, boundingRect, margin, behavior) {
  if (behavior === 'auto' || behavior === 'flip') {
    // Attempt to flip content 180 degrees
    var newPositions = getMirroredPositions(positions, margin);
...
var getBoundedPositions = function getBoundedPositions(positions, boundingRect) {
  var contentCoords = {};
  var contentRect = positions.content.rect;
  var contentX = positions.content.x;
  var contentY = positions.content.y; // Checks Horizontal Bounds
...
var positionStyleFromBounds = function positionStyleFromBounds(boundingRect, content, target, margin, behavior) {
  var targetPosition = getTargetPosition(target); // Get relative content and target positions

  var positions = getRelativePositions(content, targetPosition, margin);
  ...
  return {
    style: {
      // eslint-disable-next-line no-restricted-globals
      left: "".concat(Math.round(positions.content.x + pageXOffset), "px"),
      // eslint-disable-next-line no-restricted-globals
      top: "".concat(Math.round(positions.content.y + pageYOffset), "px")
    },
    positions: positions
  };
};
...
var _default = {
  ...
  getRectFromCoords: getRectFromCoords,
  getBounds: getBounds,
  getBoundingRect: getBoundingRect,
  mirrorAttachment: mirrorAttachment,
  coordinateAttachment: coordinateAttachment,
  getDirectionalAttachment: getDirectionalAttachment,
  getDirectionalOffset: getDirectionalOffset,
  positionStyleFromBounds: positionStyleFromBounds
};
