...
exports.showModalDomUpdates = showModalDomUpdates;
exports.hideModalDomUpdates = hideModalDomUpdates;
...
function showModalDomUpdates(modalElement, rootSelector) {
  var mainDocumentElement = document.querySelector(rootSelector);

  if (mainDocumentElement) {
    var dataOverlayCount = +mainDocumentElement.getAttribute('data-abstract-modal-overlay-count');

    if (dataOverlayCount < 1) {
      mainDocumentElement.setAttribute('inert', 'true');
    }

    mainDocumentElement.setAttribute('data-abstract-modal-overlay-count', "".concat(dataOverlayCount + 1)); // Handle focus shift for VoiceOver on iOS
...
function hideModalDomUpdates(modalTriggerElement, rootSelector) {
  ...
  if (mainDocumentElement) {
    ...
    if (dataOverlayCount === 1) {
      ...
      mainDocumentElement.setAttribute('data-abstract-modal-overlay-count', "".concat(dataOverlayCount - 1));
    }
